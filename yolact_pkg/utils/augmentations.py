...
class BackboneTransform(object):
    ...
    def __call__(self, img, masks=None, boxes=None, labels=None):
        ...
        # If we have eg RGBD imagery but want to train only on RGB just set the mean accordingly.
        if img.shape[2] != self.mean.shape[0]:
            img = img[:,:,0:self.mean.shape[0]]
...
class BaseTransform(object):
    ...
    def __init__(self, mean, std):
        self.augment = Compose([
            ...
            BackboneTransform(cfg.backbone.transform, mean, std, cfg.backbone.transform.channel_order)
        ])
...
class FastBaseTransform(torch.nn.Module):
    ...
    def __init__(self,means, std, cuda=True):
        super().__init__()
        if cuda:
