...
from data.config import set_cfg, set_dataset, cfg
...
def train(yolact_net, override_args=None):
    ...
    save_training_folder = os.path.join(args.save_folder, 'training_' + datetime.datetime.today().strftime('%Y-%m-%d-%H꞉%M:%S'))
    ...
    dataset = COCODetection(image_path=cfg.dataset.train_images,
                            ...
                            transform=SSDAugmentation(cfg.MEANS, cfg.STD))
    
    if args.validation_epoch > 0:
        setup_eval()
        val_dataset = COCODetection(image_path=cfg.dataset.valid_images,
                                    info_file=cfg.dataset.valid_info,
                                    transform=BaseTransform(cfg.MEANS, cfg.STD))
    ...
    #yolact_net(torch.zeros(1, len(cfg.backbone.transform.channel_order), cfg.max_size, cfg.max_size).cuda())
