    ...
    def init_backbone(self, path):
        ...
        for key in keys:
            if key.startswith('layer'):
                ...
                new_key = 'layers.' + str(idx-1) + key[6:]
                state_dict[new_key] = state_dict.pop(key)
        # Adding a 4th channel to the input conv
        state_dict['conv1.weight'] = torch.concat((state_dict['conv1.weight'],torch.randn((64,1,7,7), device='cpu')), dim=1)
        # state_dict['conv1.']
        # Note: Using strict=False is berry scary. Triple check this.
        self.load_state_dict(state_dict, strict=False)

class ResNetBackboneRGBD_one_layer(ResNetBackbone):
    def __init__(self, layers, dcn_layers=[0, 0, 0, 0], dcn_interval=1, atrous_layers=[], block=Bottleneck, norm_layer=nn.BatchNorm2d):
        super().__init__(layers, dcn_layers, dcn_interval, atrous_layers, block, norm_layer)

        self.conv0 = nn.Conv2d(4, 3, kernel_size=7, stride=2, padding=7, bias=True)
...
class ResNetBackboneRGBDThreeInputLayers(ResNetBackbone):
    def __init__(self, layers, dcn_layers=[0, 0, 0, 0], dcn_interval=1, atrous_layers=[], block=Bottleneck, norm_layer=nn.BatchNorm2d):
        super().__init__(layers, dcn_layers, dcn_interval, atrous_layers, block, norm_layer)

        self.conv00 = nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.bn00 = norm_layer(64)
        self.relu00 = nn.ReLU(inplace=True)

        self.conv01 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=3, bias=False)
        self.bn01 = norm_layer(64)
        self.relu01 = nn.ReLU(inplace=True)

        self.conv02 = nn.Conv2d(64, 3, kernel_size=3, stride=2, padding=3, bias=False)
        self.bn02 = norm_layer(3)
